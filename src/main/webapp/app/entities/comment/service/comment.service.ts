...
export class CommentService {
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    return Object.assign({}, comment, {
      creaionDate: comment.creaionDate?.isValid() ? comment.creaionDate.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.creaionDate = res.body.creaionDate ? dayjs(res.body.creaionDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.creaionDate = comment.creaionDate ? dayjs(comment.creaionDate) : undefined;
      });
    }
    return res;
  }
